...
/// Compute 4-bit checksum of PIN using Argon2id
/// This provides ~6.25% false positive rate (1 in 16 chance of wrong PIN passing)
pub fn compute_pin_checksum(pin: &str) -> Result<u8, Box<dyn std::error::Error>> {
    let params = Params::new(65536, 3, 4, Some(32))?;
    let argon2 = Argon2::new(argon2::Algorithm::Argon2id, Version::V0x13, params);

    let salt = b"dpg_pin_check_v2";
    let mut hash = Zeroizing::new([0u8; 32]);
    argon2.hash_password_into(pin.as_bytes(), salt, &mut *hash)?;

    // Return first 4 bits (values 0-15)
    Ok(hash[0] & 0b1111)
}
